...
def done_tasks(message): # Делает задачу выполненой
  users = read_tasks()
  telegram_id = check_users(message, users)
  conn = sqlite3.connect('baza.db')
  cursor = conn.cursor()
  ...
  if telegram_id is None or cursor.execute("SELECT active FROM users WHERE telegram_id = ?", (telegram_id,)).fetchone()[0] == 0: 
    ...
    if len(task) < 2:
      ...
      if not cursor.fetchall():
        ...
        # Если это номер задачи пытается отметить ее как выполненеая
        if task.isdigit():
            ...
            # Если это текст задачи пытается отметить ее как выполненеая
